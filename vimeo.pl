...
sub check_vimeo {
  ...
  my $raw_result = $ua->get($query)->decoded_content;
  #my $result = shift (eval { $json->utf8->decode($req->decoded_content) }); #the result in json comes in an array of 1
  eval (my $parsed_json = $json->utf8->decode($raw_result));
  return if $@;
  print (CRAP Dumper($parsed_json));
  print (CRAP ref $parsed_json);
