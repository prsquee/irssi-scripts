...
use constant IN_IRSSI => __PACKAGE__ ne 'main' || $ENV{AWL_MOCK};
...
sub awl_init {
    termsize_changed();
    update_keymap();
    print setc(), " is brand new alpha release, please report issues and do read the upgrade notes. thanks!"
	if $VERSION =~ /a$/;
}
